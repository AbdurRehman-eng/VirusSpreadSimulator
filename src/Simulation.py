...
def run_virus_simulation(r_value, simulation_days, intervention_day, intervention_strength, antidote_applied=False, antidote_day=None, antidote_effectiveness=0.0):
    ...
    for sector in initial_sectors:
        ...
        initial_count = random.randint(5, 15)
...
# Create map with focus on Islamabad
def create_islamabad_map(sectors_data, infected_people, selected_day):
    ...
    # Add infected people as dots (limit to 2000 for performance)
    MAX_DOTS = 2000
    if len(current_infected) > MAX_DOTS:
        # If there are too many, sample randomly
        current_infected = random.sample(current_infected, MAX_DOTS)
    
    # Create separate layer for infected people - faster than adding individually
...
if st.session_state.simulation_run:
    ...
    with col1:
        ...
        # Only recreate the map if the day has changed
    ...
    st.write("4. Click on sector circles for detailed information")
    st.write("5. Deploy the antidote when you're ready to start treating infected people")
    
    st.info("This simulation focuses only on Islamabad to provide better performance and visualization.")